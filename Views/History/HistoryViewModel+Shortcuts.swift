...
extension HistoryViewModel {
    ...
    // MARK: - Lock (3E-stub: replaced at merge by 3A)

    /// `ClipboardStore.toggleLock(_:)` already exists, so this is a real
    /// (if minimal) implementation rather than a no-op; 3A's dedicated
    /// `+Lock.swift` is expected to supersede it with richer behaviour
    /// (e.g. multi-selection, a confirmation for unlocking).
    func keyLock() {
        // 3E-stub: replaced at merge by 3A
        guard !isEditing else { return }
        if let item = selectedItem { store.toggleLock(item) }
    }
